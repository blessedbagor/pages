...
const ProductForm = ({ type, product, productId }: {
  ...
  // Add this useEffect to update default values when product changes
  useEffect(() => {
    if (product && type === 'Update') {
      form.reset(product); // Update the form with the new product data
    }
  }, [product, type, form]);
          ...
          <FormField
            ...
            render={() => (
              <FormItem className="w-full">
                ...
                <Card>
                  <CardContent className="space-y-2 mt-2 mmin-h-48">
